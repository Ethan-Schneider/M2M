...
def randomized_max_regret_FC_allocation(S, cost_tensor: np.ndarray, Rs : AgentLoader, start_locs: List[Tuple[int, int]], goal_locs: List[Tuple[int, int]], idx_to_task_id: Dict[int, int]) -> Tuple[List[Tuple[int, int, int, int]], float]:
    """
...
def randomized_max_regret_FC_call(S: Stats, G: Graph, Rs: AgentLoader, J: Set[Tuple], 
    ...
    tik = time.time()
    allocations, total_cost = randomized_max_regret_FC_allocation(S, cost_tensor, Rs, start_locs, goal_locs, idx_to_task_id)
    tok = time.time()

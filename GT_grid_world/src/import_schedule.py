...
# The schedule (offline) arrival path is used by the schedule-based baselines
# (TA-Hybrid, HBH+MLA*); the demand-driven queue path above is used by
# M2M / crM2M / irM2M.
...
# ---------------------------------------------------------------------------
# Schedule-based (offline, release-timed) task arrival
#
# A schedule is a table of ``(release_time, deadline, sku_id, task_type)`` rows.
# Rows are dumped into ``J`` when ``release_time <= t`` (in contrast to the
# demand-driven queue path above). Used by the schedule-based baselines
# (TA-Hybrid, HBH+MLA*). Kept alongside the queue path so both arrival models
# remain selectable after the task_queue merge.
# ---------------------------------------------------------------------------
